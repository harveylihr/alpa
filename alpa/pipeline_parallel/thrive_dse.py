...
class ThriveDseProducer(BaseRuntime):
    """Output generator for Thrive DSE format"""
    ...
    def run(self,*args, **kwargs):
        ...
        with open(filename,"w") as f:
            ...
            sys.stdout = f
            print("global.invars",self.global_invars)
            print("global_outvars",self.global_outvars)
            for stage in self.stages:
                print(stage.get_hlo_text())
                print("stage.invars", stage.invars)
                print("stage.outvars",stage.outvars)
                # print("stage.dot graph")
                # print(stage.get_dot_graph())
                print("\n\n\n")   
            sys.stdout = original_stdout
        filename =  "logs/pipeshard/sliced_per_stage_dotgraph.dot"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
        with open(filename,"w") as f:
            original_stdout = sys.stdout
            sys.stdout = f
