...
def get_inf_mlp_1_layer_computation(batch_size, input_dim, hidden_dim, output_dim):
    computation = HloComputation()
    with computation:
        x = HloParameter((batch_size, input_dim))
        # y = HloParameter((batch_size, output_dim))
...
def get_train_mlp_2_layer_computation(batch_size, input_dim, hidden_dim, output_dim):
    computation = HloComputation()
    with computation:
        x = HloParameter((batch_size, input_dim))
        y = HloParameter((batch_size, output_dim))
...
def get_inf_mlp_2_layer_bias_computation(batch_size, input_dim, hidden_dim, output_dim):
    computation = HloComputation()
    with computation:
        x = HloParameter((batch_size, input_dim))
        y = HloParameter((batch_size, output_dim))
...
def get_train_mlp_2_layer_bias_computation(batch_size, input_dim, hidden_dim, output_dim):
    computation = HloComputation()
    with computation:
        x = HloParameter((batch_size, input_dim))
        y = HloParameter((batch_size, output_dim))
...
def get_train_mlp_n_layer_computation(num_layers, batch_size, input_dim, hidden_dim, output_dim):
    computation = HloComputation()
    with computation:
        x = HloParameter((batch_size, input_dim))
        y = HloParameter((batch_size, output_dim))
...
class MLPSolverTest(unittest.TestCase):
    ...
    #         # expected = 2 * cluster_env.all_reduce_cost(hidden_dim * hidden_dim * 4, i)
    #         # assert_close(objective, expected)
    def test_inf_mlp_1_layer_2DPEmesh(self):
        # Build Hlo Computation
        batch_size = 1024
        hidden_dim = 1024
        mem_per_device_MB = 10
        computation = get_inf_mlp_1_layer_computation(batch_size, hidden_dim,
            hidden_dim, hidden_dim)
        solver_option = SolverOption(
                                    [64],
                                    2)
        # Test different device meshes
        for i, mesh_shape in enumerate([(16,1), (1,16), (2,8), (8,2), (4,4)]):
            print("####### Mesh shape: ",mesh_shape," #######")
            device_mesh = np.arange(np.prod(mesh_shape)).reshape(mesh_shape)
            cluster_env = ClusterEnvironment(device_mesh, [1, 1], [1, 1],
                                             memory_per_device=mem_per_device_MB * MB)
            objective = solve_auto_sharding(computation, cluster_env,solver_option)
    
    def test_inf_mlp_2_layer_2DPEmesh(self):
        # Build Hlo Computation
        batch_size = 1024
        hidden_dim = 1024
        mem_per_device_MB = 1
        ...
        solver_option = SolverOption(
                                    ...
                                    2)
        # Test different device meshes
        for i, mesh_shape in enumerate([ (4,4)]):
            print("####### Mesh shape: ",mesh_shape," #######")
            device_mesh = np.arange(np.prod(mesh_shape)).reshape(mesh_shape)
            cluster_env = ClusterEnvironment(device_mesh, [1, 1], [1, 1],
                                             memory_per_device=mem_per_device_MB * MB)
    ...
    def test_inf_mlp_2_layer_bias_2DPEmesh(self):
        # Build Hlo Computation
        batch_size = 1024
        hidden_dim = 1024
        mem_per_device_MB = 2
        computation = get_inf_mlp_2_layer_bias_computation(batch_size, hidden_dim,
            hidden_dim, hidden_dim)
        solver_option = SolverOption(
                                    [64],
                                    2)
    ...
    def test_train_mlp_2_layer_2DPEmesh(self):
        # Build Hlo Computation
        batch_size = 1024
        hidden_dim = 1024
        mem_per_device_MB = 10
        computation = get_train_mlp_2_layer_computation(batch_size, hidden_dim,
            hidden_dim, hidden_dim)
        solver_option = SolverOption(
                                    [64],
                                    2)
...
def suite():
    ...
    # HLI: test inference
    # suite.addTest(MLPSolverTest('test_inf_mlp_1_layer_2DPEmesh'))

    # suite.addTest(MLPSolverTest('test_inf_mlp_2_layer_2DPEmesh'))
    suite.addTest(MLPSolverTest('test_inf_mlp_2_layer_bias_2DPEmesh'))
    # suite.addTest(MLPSolverTest('test_train_mlp_2_layer_2DPEmesh'))
